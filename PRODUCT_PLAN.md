...
GitHub Pages 保留为静态互动 Demo。实时版本已部署于 `https://roundtable.xiaoshixuji.xyz/`，采用 Ubuntu + 独立 Node 22 服务 + Nginx HTTPS，前后端同域。后端直接调用知乎官方搜索与直答 HTTP API，Access Secret 仅存储在服务器受限目录；当前包含 Nginx 访问限流、问题缓存、并发限制、systemd 自动重启和日志。后续仍需完善内容审核、额度监控和多用户会话持久化。
...
当前版本用于验证核心交互。知乎来源基于公开搜索摘要，立场生成不等同于对完整原回答的事实裁决；关键论据应通过原文链接核验。GitHub Pages 是静态互动演示；独立子域名提供公网实时检索，本地仍支持知乎 CLI。缓存与对话尚未持久化，重启服务或刷新页面可能需要重新开始圆桌。
